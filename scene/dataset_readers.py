...
def readKaggleTestCameras(test_poses_file):
    import csv
    cam_infos = []
    if not os.path.exists(test_poses_file):
        return cam_infos
    with open(test_poses_file, 'r') as f:
        lines = f.readlines()
        if len(lines) <= 1:
            return cam_infos
        for idx, line in enumerate(lines[1:]):
            ...
            if len(parts) < 14:
                continue
            image_name, qw, qx, qy, qz, tx, ty, tz, fx, fy, cx, cy, width, height = parts
            uid = idx + 99999
